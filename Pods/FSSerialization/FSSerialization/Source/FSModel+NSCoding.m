...
static NSString * const FSModelVersionKey = @"FSModelVersion";

// Used to cache the reflection performed in +allowedSecureCodingClassesByPropertyKey.
static void *FSModelCachedAllowedClassesKey = &FSModelCachedAllowedClassesKey;
...
static NSSet *encodablePropertyKeysForClass(Class modelClass) {
	return [[modelClass encodingBehaviorsByPropertyKey] keysOfEntriesPassingTest:^ BOOL (NSString *propertyKey, NSNumber *behavior, BOOL *stop) {
		return behavior.unsignedIntegerValue != FSModelEncodingBehaviorExcluded;
	}];
}
...
@implementation FSModel (NSCoding)
...
+ (NSDictionary *)encodingBehaviorsByPropertyKey {
	...
	for (NSString *key in propertyKeys) {
		...
		FSModelEncodingBehavior behavior = (attributes->weak ? FSModelEncodingBehaviorConditional : FSModelEncodingBehaviorUnconditional);
...
+ (NSDictionary *)allowedSecureCodingClassesByPropertyKey {
	NSDictionary *cachedClasses = objc_getAssociatedObject(self, FSModelCachedAllowedClassesKey);
	...
	objc_setAssociatedObject(self, FSModelCachedAllowedClassesKey, allowedClasses, OBJC_ASSOCIATION_COPY);
...
- (id)decodeValueForKey:(NSString *)key withCoder:(NSCoder *)coder modelVersion:(NSUInteger)modelVersion {
	...
	SEL selector = FSSelectorWithCapitalizedKeyPattern("decode", key, "WithCoder:modelVersion:");
...
- (instancetype)initWithCoder:(NSCoder *)coder {
	...
	if (requiresSecureCoding) {
		version = [coder decodeObjectOfClass:NSNumber.class forKey:FSModelVersionKey];
	} else {
		version = [coder decodeObjectForKey:FSModelVersionKey];
	}
...
- (void)encodeWithCoder:(NSCoder *)coder {
	...
	[coder encodeObject:@(self.class.modelVersion) forKey:FSModelVersionKey];
	...
	[self.dictionaryValue enumerateKeysAndObjectsUsingBlock:^(NSString *key, id value, BOOL *stop) {
		@try {
			...
			switch ([encodingBehaviors[key] unsignedIntegerValue]) {
				...
				case FSModelEncodingBehaviorExcluded:
					break;
					
				case FSModelEncodingBehaviorUnconditional:
					[coder encodeObject:value forKey:key];
					break;
					
				case FSModelEncodingBehaviorConditional:
...
@implementation FSModel (OldArchiveSupport)
