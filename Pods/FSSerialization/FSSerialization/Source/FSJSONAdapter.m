...
#import "NSDictionary+FSJSONKeyPath.h"
...
NSString * const FSJSONAdapterErrorDomain = @"FSJSONAdapterErrorDomain";
const NSInteger FSJSONAdapterErrorNoClassFound = 2;
const NSInteger FSJSONAdapterErrorInvalidJSONDictionary = 3;
const NSInteger FSJSONAdapterErrorInvalidJSONMapping = 4;

// An exception was thrown and caught.
const NSInteger FSJSONAdapterErrorExceptionThrown = 1;

// Associated with the NSException that was caught.
NSString * const FSJSONAdapterThrownExceptionErrorKey = @"FSJSONAdapterThrownException";

@interface FSJSONAdapter ()

// The FSModel subclass being parsed, or the class of `model` if parsing has
// completed.
...
//              to <FSJSONSerializing>. This argument must not be nil.
...
- (FSJSONAdapter *)JSONAdapterForModelClass:(Class)modelClass error:(NSError **)error;
...
@implementation FSJSONAdapter

#pragma mark Convenience methods

+ (id)modelOfClass:(Class)modelClass fromJSONDictionary:(NSDictionary *)JSONDictionary error:(NSError **)error {
	FSJSONAdapter *adapter = [[self alloc] initWithModelClass:modelClass];
...
+ (NSArray *)modelsOfClass:(Class)modelClass fromJSONArray:(NSArray *)JSONArray error:(NSError **)error {
	if (JSONArray == nil || ![JSONArray isKindOfClass:NSArray.class]) {
		if (error != NULL) {
			...
			*error = [NSError errorWithDomain:FSJSONAdapterErrorDomain code:FSJSONAdapterErrorInvalidJSONDictionary userInfo:userInfo];
		}
		return nil;
	}
	...
	for (NSDictionary *JSONDictionary in JSONArray){
		FSModel *model = [self modelOfClass:modelClass fromJSONDictionary:JSONDictionary error:error];
...
+ (NSDictionary *)JSONDictionaryFromModel:(id<FSJSONSerializing>)model error:(NSError **)error {
	FSJSONAdapter *adapter = [[self alloc] initWithModelClass:model.class];
...
+ (NSArray *)JSONArrayFromModels:(NSArray *)models error:(NSError **)error {
	...
	for (FSModel<FSJSONSerializing> *model in models) {
...
- (id)initWithModelClass:(Class)modelClass {
	...
	NSParameterAssert([modelClass conformsToProtocol:@protocol(FSJSONSerializing)]);
...
- (NSDictionary *)JSONDictionaryFromModel:(id<FSJSONSerializing>)model error:(NSError **)error {
	...
	if (self.modelClass != model.class) {
		FSJSONAdapter *otherAdapter = [self JSONAdapterForModelClass:model.class error:error];
	...
	[dictionaryValue enumerateKeysAndObjectsUsingBlock:^(NSString *propertyKey, id value, BOOL *stop) {
		...
		if ([transformer.class allowsReverseTransformation]) {
			...
			if ([transformer respondsToSelector:@selector(reverseTransformedValue:success:error:)]) {
				id<FSTransformerErrorHandling> errorHandlingTransformer = (id)transformer;
	...
	if ([self.modelClass respondsToSelector:@selector(classForParsingJSONDictionary:)]) {
		...
		if (class == nil) {
			if (error != NULL) {
				...
				*error = [NSError errorWithDomain:FSJSONAdapterErrorDomain code:FSJSONAdapterErrorNoClassFound userInfo:userInfo];
			}
		...
		if (class != self.modelClass) {
			NSAssert([class conformsToProtocol:@protocol(FSJSONSerializing)], @"Class %@ returned from +classForParsingJSONDictionary: does not conform to <FSJSONSerializing>", class);

			FSJSONAdapter *otherAdapter = [self JSONAdapterForModelClass:class error:error];
	...
	for (NSString *propertyKey in [self.modelClass propertyKeys]) {
		...
		if ([JSONKeyPaths isKindOfClass:NSArray.class]) {
			...
			for (NSString *keyPath in JSONKeyPaths) {
				BOOL success = NO;
				id value = [JSONDictionary fs_valueForJSONKeyPath:keyPath success:&success error:error];
			...
			BOOL success = NO;
			value = [JSONDictionary fs_valueForJSONKeyPath:JSONKeyPaths success:&success error:error];
		...
		@try {
			...
			if (error != NULL) {
				NSDictionary *userInfo = @{
					...
					FSJSONAdapterThrownExceptionErrorKey: ex
				};

				*error = [NSError errorWithDomain:FSJSONAdapterErrorDomain code:FSJSONAdapterErrorExceptionThrown userInfo:userInfo];
			}
...
+ (NSDictionary *)valueTransformersForModelClass:(Class)modelClass {
	...
	for (NSString *key in [modelClass propertyKeys]) {
		SEL selector = FSSelectorWithKeyPattern(key, "JSONTransformer");
		...
		if (*(attributes->type) == *(@encode(id))) {
			...
			// For user-defined FSModel, try parse it with dictionaryTransformer.
			if (nil == transformer && [propertyClass conformsToProtocol:@protocol(FSJSONSerializing)]) {
				transformer = [self dictionaryTransformerWithModelClass:propertyClass];
			}
			
			if (transformer == nil) transformer = [NSValueTransformer fs_validatingTransformerForClass:propertyClass ?: NSObject.class];
		} else {
			transformer = [self transformerForModelPropertiesOfObjCType:attributes->type] ?: [NSValueTransformer fs_validatingTransformerForClass:NSValue.class];
		}
...
- (FSJSONAdapter *)JSONAdapterForModelClass:(Class)modelClass error:(NSError **)error {
	...
	@synchronized(self) {
		FSJSONAdapter *result = [self.JSONAdaptersByModelClass objectForKey:modelClass];
...
- (NSSet *)serializablePropertyKeys:(NSSet *)propertyKeys forModel:(id<FSJSONSerializing>)model {
    NSMutableSet *ms = propertyKeys.mutableCopy;
    NSDictionary *modelDictValue = [model dictionaryValue];
    for (NSString *key in [ms copy]) {
        id val = [modelDictValue valueForKey:key];
        if ([[NSNull null] isEqual:val] || [@"" isEqualToString:val] || [val isEqual:@0]) { // -dictionaryValue nil value is represented by NSNull
            [ms removeObject:key];
        }
    }
...
+ (NSValueTransformer *)transformerForModelPropertiesOfClass:(Class)modelClass {
	...
	SEL selector = FSSelectorWithKeyPattern(NSStringFromClass(modelClass), "JSONTransformer");
...
+ (NSValueTransformer *)transformerForModelPropertiesOfObjCType:(const char *)objCType {
	...
	if (strcmp(objCType, @encode(BOOL)) == 0) {
		return [NSValueTransformer valueTransformerForName:FSBooleanValueTransformerName];
	}
...
@implementation FSJSONAdapter (ValueTransformers)

+ (NSValueTransformer<FSTransformerErrorHandling> *)dictionaryTransformerWithModelClass:(Class)modelClass {
	NSParameterAssert([modelClass conformsToProtocol:@protocol(FSModel)]);
	NSParameterAssert([modelClass conformsToProtocol:@protocol(FSJSONSerializing)]);
	__block FSJSONAdapter *adapter;
	...
	return [FSValueTransformer
			...
			if (![JSONDictionary isKindOfClass:NSDictionary.class]) {
				if (error != NULL) {
					NSDictionary *userInfo = @{
						...
						FSTransformerErrorHandlingInputValueErrorKey : JSONDictionary
					};
		...
		reverseBlock:^ NSDictionary * (id model, BOOL *success, NSError **error) {
			...
			if (![model conformsToProtocol:@protocol(FSModel)] || ![model conformsToProtocol:@protocol(FSJSONSerializing)]) {
				if (error != NULL) {
					NSDictionary *userInfo = @{
						NSLocalizedDescriptionKey: NSLocalizedString(@"Could not convert model object to JSON dictionary", @""),
						NSLocalizedFailureReasonErrorKey: [NSString stringWithFormat:NSLocalizedString(@"Expected a FSModel object conforming to <FSJSONSerializing>, got: %@.", @""), model],
						FSTransformerErrorHandlingInputValueErrorKey : model
					};
...
+ (NSValueTransformer<FSTransformerErrorHandling> *)arrayTransformerWithModelClass:(Class)modelClass {
	id<FSTransformerErrorHandling> dictionaryTransformer = [self dictionaryTransformerWithModelClass:modelClass];
	...
	return [FSValueTransformer
			...
			if (![dictionaries isKindOfClass:NSArray.class]) {
				if (error != NULL) {
					NSDictionary *userInfo = @{
						...
						FSTransformerErrorHandlingInputValueErrorKey : dictionaries
					};
		...
		reverseBlock:^ id (NSArray *models, BOOL *success, NSError **error) {
			...
			if (![models isKindOfClass:NSArray.class]) {
				if (error != NULL) {
					NSDictionary *userInfo = @{
						...
						FSTransformerErrorHandlingInputValueErrorKey : models
					};
			...
			for (id model in models) {
				...
				if (![model isKindOfClass:FSModel.class]) {
					if (error != NULL) {
						NSDictionary *userInfo = @{
							NSLocalizedDescriptionKey: NSLocalizedString(@"Could not convert JSON array to model array", @""),
							NSLocalizedFailureReasonErrorKey: [NSString stringWithFormat:NSLocalizedString(@"Expected a FSModel or an NSNull, got: %@.", @""), model],
							FSTransformerErrorHandlingInputValueErrorKey : model
						};
...
+ (NSValueTransformer *)NSURLJSONTransformer {
	return [NSValueTransformer valueTransformerForName:FSURLValueTransformerName];
}

+ (NSValueTransformer *)NSUUIDJSONTransformer {
	return [NSValueTransformer valueTransformerForName:FSUUIDValueTransformerName];
}
