...
fs_propertyAttributes *fs_copyPropertyAttributes (objc_property_t property) {
    ...
    fs_propertyAttributes *attributes = calloc(1, sizeof(fs_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate fs_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        case 'C':
            attributes->memoryManagementPolicy = fs_propertyMemoryManagementPolicyCopy;
            break;

        case '&':
            attributes->memoryManagementPolicy = fs_propertyMemoryManagementPolicyRetain;
            break;
