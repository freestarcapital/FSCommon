...
#define onExit \
    try {} @finally {} \
    __strong fs_cleanupBlock_t metamacro_concat(fs_exitBlock_, __LINE__) __attribute__((cleanup(fs_executeCleanupBlock), unused)) = ^
...
#define weakify(...) \
    try {} @finally {} \
    metamacro_foreach_cxt(fs_weakify_,, __weak, __VA_ARGS__)
...
#define unsafeify(...) \
    try {} @finally {} \
    metamacro_foreach_cxt(fs_weakify_,, __unsafe_unretained, __VA_ARGS__)
...
#define strongify(...) \
    ...
    metamacro_foreach(fs_strongify_,, __VA_ARGS__) \
    _Pragma("clang diagnostic pop")

/*** implementation details follow ***/
typedef void (^fs_cleanupBlock_t)(void);

void fs_executeCleanupBlock (__strong fs_cleanupBlock_t *block);

#define fs_weakify_(INDEX, CONTEXT, VAR) \
    CONTEXT __typeof__(VAR) metamacro_concat(VAR, _weak_) = (VAR);

#define fs_strongify_(INDEX, VAR) \
