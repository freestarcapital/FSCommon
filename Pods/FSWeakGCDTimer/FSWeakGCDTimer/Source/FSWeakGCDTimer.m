...
#error FSWeakGCDTimer is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif
...
@interface FSWeakGCDTimer ()
{
    struct
    {
        atomic_bool timerIsInvalidated;
    } _timerFlags;
}
             ...
             dispatchQueue:(dispatch_queue_t)dispatchQueue
{
    ...
    if ((self = [super init]))
    {
        ...
        NSString *privateQueueName = [NSString stringWithFormat:@"io.freestar.mobile.timer.%p", self];
...
+ (instancetype)scheduledTimerWithTimeInterval:(NSTimeInterval)timeInterval
                                 ...
                                 dispatchQueue:(dispatch_queue_t)dispatchQueue
{
    FSWeakGCDTimer *timer = [[self alloc] initWithTimeInterval:timeInterval
                                                     target:target
                                                   selector:selector
                                                   userInfo:userInfo
                                                    repeats:repeats
...
- (void)schedule
{
    ...
    __weak FSWeakGCDTimer *weakSelf = self;
...
- (void)invalidate
{
    ...
    if (!atomic_fetch_or(&_timerFlags.timerIsInvalidated, true))
    {
...
- (void)timerFired
{
    ...
    if (atomic_fetch_or(&_timerFlags.timerIsInvalidated, false))
    {
        return;
    }
